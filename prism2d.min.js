...
(function (global) {
  ...
  prism._keysNew = {};
  prism._keysNewUp = {};
  prism._mouse = { x: 0, y: 0, down: false, justPressed: false, justReleased: false };
...
(function (global) {
  ...
  prism._resize = function () {
    ...
    } else if (prism._mode === "pixel") {
      canvas.width = prism.width;
      canvas.height = prism.height;
    } else {
      canvas.width = Math.round(prism.width * dpr);
      canvas.height = Math.round(prism.height * dpr);
      prism.ctx.setTransform(dpr, 0, 0, dpr, 0, 0);
...
(function (global) {
  ...
  prism._getCanvasPos = function (cx, cy) {
    ...
    return {
      x: (cx - rect.left) * (prism.canvas.width / rect.width) / prism._dpr,
      y: (cy - rect.top) * (prism.canvas.height / rect.height) / prism._dpr
    };
  };
  ...
  prism._updateTouches = function (list) {
    ...
    for (var i = 0; i < list.length; i++) {
      ...
      prism._touches.push({ x: p.x, y: p.y, id: t.identifier });
    }
  };
  ...
  prism.axisX = function () {
    var v = 0;
    if (prism.key("ArrowLeft") || prism.key("KeyA")) v -= 1;
    if (prism.key("ArrowRight") || prism.key("KeyD")) v += 1;
    return v;
  };

  prism.axisY = function () {
    var v = 0;
    if (prism.key("ArrowUp") || prism.key("KeyW")) v -= 1;
    if (prism.key("ArrowDown") || prism.key("KeyS")) v += 1;
    return v;
  };
  ...
  prism.mouse = {
    ...
    worldX: function () {
      ...
      return c ? (prism._mouse.x - prism.width * 0.5 - c.offsetX) / c.zoom + c.x : prism._mouse.x;
    },
    worldY: function () {
      var c = prism._activeCamera;
      return c ? (prism._mouse.y - prism.height * 0.5 - c.offsetY) / c.zoom + c.y : prism._mouse.y;
    }
  };
...
(function (global) {
  ...
  function makeShape(type, config) {
    ...
    obj.size = function (w, h) { obj.w = w; obj.h = h; return obj; };
    ...
    obj.jump = function (force, requireGround) {
      if (requireGround !== false) {
        var canJump = obj.grounded || obj.coyote > 0;
  ...
  prism.render = function (obj) {
    ...
    if (hasTransform) {
      var ctx = prism.ctx;
      ctx.save();
      ctx.translate(prism.snap(px + obj.w * 0.5), prism.snap(py + obj.h * 0.5));
      if (obj.angle) ctx.rotate(obj.angle);
      ctx.scale(obj._sx * (obj._flipX ? -1 : 1), obj._sy * (obj._flipY ? -1 : 1));
      ctx.translate(-obj.w * 0.5, -obj.h * 0.5);
      ...
      if (obj.type === "rect") {
        ...
        var img = prism._images[obj.image];
        if (img) {
          if (obj.animated) {
            ctx.drawImage(img, obj._currentFrame * obj.frameW, obj.frameY * obj.frameH, obj.frameW, obj.frameH, 0, 0, obj.w, obj.h);
          } else {
    ...
    var sx = prism.snap(px);
    var sy = prism.snap(py);
    ...
    if (obj.type === "rect") {
      ...
      var img2 = prism._images[obj.image];
      if (!img2) return prism;
      if (obj.animated) {
        prism.ctx.drawImage(img2, obj._currentFrame * obj.frameW, obj.frameY * obj.frameH, obj.frameW, obj.frameH, sx, sy, obj.w, obj.h);
      } else {
        prism.ctx.drawImage(img2, sx, sy, obj.w, obj.h);
      }
    }
    return prism;
  };
...
(function (global) {
  ...
  prism.drawImg = function (name, x, y, w, h) {
    var img = prism._images[name];
    if (!img) return prism;
    if (w !== undefined) prism.ctx.drawImage(img, prism.snap(x), prism.snap(y), w, h);
    else prism.ctx.drawImage(img, prism.snap(x), prism.snap(y));
    return prism;
  };

  prism.sprite = function (name, x, y) {
    var img = prism._images[name];
    var w = img ? img.width : 32;
    var h = img ? img.height : 32;
...
(function (global) {
  ...
  prism.solve = function (moving, solid, opts) {
    ...
    if (solid.oneWay) {
      ...
      if (prevBottom > solid.y + 1 || moving.vy < 0) return null;
  ...
  prism.sweptAABB = function (a, vx, vy, b) {
    ...
    if (vx === 0) { xEntry = -Infinity; xExit = Infinity; }
    else { xEntry = xInvEntry / vx; xExit = xInvExit / vx; }
    if (vy === 0) { yEntry = -Infinity; yExit = Infinity; }
...
(function (global) {
  ...
  prism.drawTilemap = function (map, cam) {
    ...
    var img = prism._images[map.image];
    if (!img) return;
    ...
    if (cam) {
      ...
      sc = Math.floor((cam.x - hw - map.x) / tw) - 1;
      ec = Math.ceil((cam.x + hw - map.x) / tw) + 1;
      sr = Math.floor((cam.y - hh - map.y) / th) - 1;
      er = Math.ceil((cam.y + hh - map.y) / th) + 1;
      if (sc < 0) sc = 0;
      if (sr < 0) sr = 0;
